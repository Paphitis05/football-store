...
def add_customer():
    ...
    cursor.execute(query, (customer_id, request.form['Customer_Name'], request.form['email'], request.form['Customer_address'], request.form['Customer_Phone']))
    conn.commit()
    conn.close()
...
def add_item():
    ...
    cursor.execute("SELECT name FROM item WHERE name = %s", (item_name,))
    if cursor.fetchone():
        conn.close()
    ...
    cursor.execute(query, (item_name, request.form['description'], request.form['price'], request.form['availability'], request.form['category'], request.form['color'], request.form['size']))
    conn.commit()
    conn.close()
...
def create_order():
    ...
    cursor.execute("SELECT order_code FROM orders WHERE order_code = %s", (order_code,))
    if cursor.fetchone():
        conn.close()
        flash('Order with the same code already exists.', 'error')
        return redirect(url_for('data_entry'))

    query = "INSERT INTO orders (order_code, Customer_ID, sales_person_id, items_qty) VALUES (%s, %s, %s, 0)"
    cursor.execute(query, (order_code, request.form['Customer_ID'], request.form['sales_person_id']))
    conn.commit()
    new_order_id = cursor.lastrowid
    conn.close()
    flash('Order created successfully! You can now add items.', 'success')
