...
def query1():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    query = "SELECT Customer_ID, Customer_Name, email, Customer_Phone FROM customer WHERE Customer_ID = %s OR Customer_Name = %s"
...
def query2():
    ...
    cursor = conn.cursor()
    query = """
    SELECT tc.Team_Name, COUNT(p.player_id) AS NumberOfPlayers
    FROM team_customer tc
    JOIN player p ON tc.Customer_ID = p.team_customer_id
    WHERE tc.Customer_ID = %s OR tc.Team_Name = %s
...
def query3():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    query = "SELECT name, description, category FROM item WHERE item_id = %s OR name = %s"
...
def query4():
    conn = get_db_connection()
    cursor = conn.cursor()
    query = "SELECT item_id, name, price, availability FROM item WHERE price BETWEEN 3.00 AND 15.00"
...
def query5():
    conn = get_db_connection()
    cursor = conn.cursor()
    query = "SELECT Customer_ID, Customer_Name, Customer_address FROM customer WHERE Customer_address LIKE '%Starford%' OR Customer_address LIKE '%Liverpool%'"
    cursor.execute(query)
    result = cursor.fetchall()
    conn.close()
    return render_template('queries.html', query_result=result)
...
def query6():
    conn = get_db_connection()
    cursor = conn.cursor()
    # This query calculates the average sale amount for each salesperson,
    # considering the quantity of items sold and any applicable discounts for team customers.
    query = """
    SELECT 
        sp.Name, 
        AVG(oi.quantity * i.price * (1 - COALESCE(tc.Discount, 0))) AS AverageSale
...
def query7():
    ...
    WITH OrderTotals AS (
        SELECT 
            ...
            SUM(oi.quantity * i.price * (1 - COALESCE(tc.Discount, 0))) as total_value
        FROM orders o
        JOIN order_item oi ON o.Order_ID = oi.Order_ID
        JOIN item i ON oi.item_id = i.item_id
        JOIN customer c ON o.Customer_ID = c.Customer_ID
        ...
        LEFT JOIN team_customer tc ON c.Customer_ID = tc.Customer_ID
        GROUP BY o.Order_ID, sp.ID, c.Customer_ID
    )
    SELECT 
        ot.SalesPerson_Name,
        ...
        ot.total_value
    FROM OrderTotals ot
    JOIN sales_person sp ON ot.SalesPerson_ID = sp.ID
